...
export default async function ObservePage({ params }: { params: Promise<{ lessonId: string }> }) {
  ...
  return <LevelGate unitIndex={units.indexOf(unit)}><LessonScreen eyebrow="AMATI REFERENSI" title={`Bentuk huruf ${sign.symbol}`} description={sign.instruction} back={{ href: `/lesson/${lesson.id}`, label: "Pengantar materi" }}>
