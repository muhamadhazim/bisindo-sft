...
export async function GET(_request: Request, context: RouteContext<"/verify/[code]">) {
  ...
  return new NextResponse(`<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Verifikasi Sertifikat Sinyal</title></head><body style="font-family:system-ui;max-width:38rem;margin:4rem auto;padding:1.5rem;color:#142e47"><p>SINYAL · VERIFIKASI SERTIFIKAT</p><h1>${certificate.is_valid ? "Sertifikat valid" : "Sertifikat dicabut"}</h1><p><strong>${escapeHtml(certificate.recipient_name)}</strong></p><p>Sertifikat Penyelesaian Pembelajaran Alfabet BISINDO · ${escapeHtml(certificate.curriculum_version)}</p><p>Diterbitkan ${new Intl.DateTimeFormat("id-ID", { dateStyle: "long" }).format(new Date(certificate.issued_at))}</p></body></html>`, { headers: { "content-type": "text/html; charset=utf-8", "x-content-type-options": "nosniff" } });
}
