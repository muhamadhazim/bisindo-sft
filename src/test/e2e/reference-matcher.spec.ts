...
// This archived experiment targets the former Sanjaya reference source.
const legacySigns = signContents.map(sign => ({...sign, sourceId: model.sourceId}));
...
test("source exemplars match their label and reject a different supported target; not an accuracy evaluation", () => {
  ...
  for (const p of model.prototypes) {
    const target = legacySigns.find(s => s.id === p.signId)!;
    const side = p.side as "LEFT" | "RIGHT";
    expect(engine.classify(p.vector, side, target).candidate).toBe("MATCH");
    const other = legacySigns.find(s => s.id !== p.signId && model.prototypes.some(q => q.side === side && q.signId === s.id))!;
