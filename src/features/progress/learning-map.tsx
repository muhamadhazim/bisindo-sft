  ...
  return <div className="shell map-shell">
    ...
    <section className="map-content" aria-labelledby="map-heading">
      ...
      <div className="learning-map" aria-busy={status === "loading"}>
        ...
        <nav className="map-stops" aria-label="Pilih kelompok alfabet">{mapStops.map((stop, index) => locked(index) ? <span key={stop.id} className={`map-stop map-stop-${index} is-locked`} aria-disabled="true">{stopContent(stop.title, stop.symbol, index)}</span> : <Link key={stop.id} href={stop.href} className={`map-stop map-stop-${index}`}>{stopContent(stop.title, stop.symbol, index)}</Link>)}</nav>
