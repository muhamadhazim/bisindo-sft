...
export function CertificateDownload({ certificate }: { certificate: Certificate }) {
  ...
  return <PDFDownloadLink className="button secondary" document={document} fileName={`sertifikat-handsign-${certificate.verification_code}.pdf`}>{({ loading }) => loading ? "Membuat PDF…" : "Unduh PDF"}</PDFDownloadLink>;
}
